...
- [ ] Given the session token has expired or been invalidated, when the app launches, then a silent re-authentication is attempted before falling back to onboarding
...
**Action:** If onboarding has been completed (flag persisted locally), skip directly to Home screen on subsequent launches. If the flag is missing but a valid anonymous session exists, skip onboarding.
**Error Message:** N/A.
...
**Action:** No network calls for account creation occur until the user taps "Start." The privacy screen is a gate.
**Error Message:** N/A.
...
**Action:** The system requests an anonymous credential from the auth provider. The resulting `user_id` is an opaque, non-reversible identifier.
**Error Message:** N/A.
...
| 7 | App backgrounded during account creation | On return to foreground, check if creation completed; if not, retry silently or show error |
