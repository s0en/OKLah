...
- Q: What is the nature of the identifier the user selects in BR-ONB-004? → A: Anonymous display name (e.g., "Cheerful Sunrise") — cosmetic only, shown on Home screen.
- Q: Should the motivational message on the Home screen rotate, and what is its source? → A: Removed. Requirement deferred to a future iteration.
- Q: What level of abuse prevention during anonymous account creation for MVP? → A: Device integrity check only (Play Integrity / App Attest) before account creation.
- Q: When a new anonymous account is created after re-auth failure, what happens to old local data? → A: Migrate local data (check-ins, streak) to new account. UX must NOT feel like re-onboarding — skip the privacy screen and present only the display name selection ("choose a new alias").
- Q: Which store is the source of truth for `onboarding_complete`? → A: Local device is source of truth. Backend is best-effort sync.
...
**Action:** If onboarding has been completed (flag persisted locally), skip directly to Home screen on subsequent launches. If the flag is missing but a valid anonymous session exists, skip onboarding and navigate to Home screen. The **local device** is the source of truth for `onboarding_complete`; the backend copy is a best-effort sync and is never used to override the local value.
**Error Message:** N/A.
...
**Description:** The anonymous user identifier is generated by the authentication service, not derived from any device identifier that could be used for cross-app tracking. Additionally, the system generates a cosmetic display name for the user's profile.
**Condition:** Account creation.
**Action:** The system requests an anonymous credential from the auth provider. The resulting `user_id` is an opaque, non-reversible identifier. After account creation, the system generates three fictional display names randomly selected from a library of terms associated with happiness and motivation (e.g., "Cheerful Sunrise," "Bright Horizon," "Joyful Wave"). The user selects one of these three options as their cosmetic display name. This name is for display purposes only (shown on the Home screen) and is not used for authentication or identification.
**Error Message:** N/A.
...
erDiagram
    AnonymousUser {
        ...
        string display_name "Cosmetic display name chosen by user"
...
| display_name | string | Yes | User-selected | Cosmetic display name chosen from three generated options (e.g., "Cheerful Sunrise"). For display on Home screen only. |
| created_at | datetime | Yes | Server timestamp | UTC timestamp of account creation |
| onboarding_complete | boolean | Yes | false | Best-effort sync of local flag. Not authoritative — local device value takes precedence. |

#### LocalSession (Device — Source of Truth)
...
| onboarding_complete | boolean | Yes | false | **Source of truth.** Local flag to skip onboarding on relaunch. Overrides backend value if they disagree. |
...
### 6.2 Screen: Display Name Selection

**Purpose:** Let the user choose a cosmetic anonymous display name from three system-generated options.
**Entry Point:** Successful account creation.
**Exit Points:** Home screen (after selecting a name).
...
**Fields:** None (selection only, no free-text input).

**Actions:**

| Action | Trigger | Behavior |
|--------|---------|----------|
| Select name | Tap one of three options | Persist chosen `display_name` locally and to backend → navigate to Home screen |

**Design Constraints:**

- Three options are generated randomly from a curated library of happiness/motivation terms.
- No option to type a custom name (privacy-first: avoids PII entry).
- Selection is required — the user cannot skip this step.

---

### 6.3 Screen: Loading / Account Creation
...
flowchart TD
    ...
    SILENT_REAUTH -->|Failure| REAUTH_RECOVERY[Create New Account Silently]
    REAUTH_RECOVERY --> MIGRATE_DATA[Migrate Local Data to New user_id]
    MIGRATE_DATA --> NAME_SELECTION
    CHECK_SESSION -->|No| PRIVACY_SCREEN[Privacy Statement Screen]
    PRIVACY_SCREEN -->|User taps Start| LOADING[Loading State]
    LOADING --> INTEGRITY_CHECK[Device Integrity Check]
    INTEGRITY_CHECK -->|Pass| CREATE_ACCOUNT[Create Anonymous Account]
    INTEGRITY_CHECK -->|Fail| INTEGRITY_ERROR[Device Not Supported Error]
    CREATE_ACCOUNT -->|Success| NAME_SELECTION[Display Name Selection Screen]
    NAME_SELECTION -->|User selects name| SET_FLAGS[Set onboarding_complete flags]
...
| 4 | System | Run device integrity check | Verify device via Play Integrity / App Attest |
| 5 | System | Integrity check passes | Call anonymous auth to create account |
| 6 | System | Account created | Persist session locally, generate three display name options |
| 7 | User | Select display name | Choose one of three generated cosmetic names |
| 8 | System | Persist display name | Save chosen name locally and to backend, set onboarding_complete = true |
| 9 | System | Fire `onboarding_start` and `onboarding_complete` events | Analytics recorded |
| 10 | System | Navigate to Home | Home screen displayed with check-in action and display name visible |
...
stateDiagram-v2
    ...
    CREATING_ACCOUNT --> NAME_SELECTION : Account created successfully
    CREATING_ACCOUNT --> ERROR : Account creation failed
    ERROR --> CREATING_ACCOUNT : User taps Try Again
    NAME_SELECTION --> ONBOARDED : User selects display name
    ...
    SESSION_EXPIRED --> REAUTH_RECOVERY : Silent re-auth failed (all 3 attempts)
    REAUTH_RECOVERY --> NAME_SELECTION : New account created, local data migrated
```
...
| NAME_SELECTION | Account created; user is choosing a display name (also used during re-auth recovery) |
| REAUTH_RECOVERY | Silent re-auth failed; new account created silently and local data migrated |
| ONBOARDED | Anonymous account exists, display name chosen, and session is valid |
...
| ERR-ONB-003 | Silent re-auth failed on relaunch | N/A (no message — handled transparently) | Create new anonymous account silently → skip privacy screen → show display name selection only ("Choose a new alias"). Migrate all local data (check-ins, streak) to the new `user_id`. User must not perceive this as re-onboarding. |
| ERR-ONB-004 | Device integrity check failed | "This device is not supported. Please try again on a different device." | Show error message with no retry (integrity failure is deterministic for a given device state) |
...
| 8 | Silent re-auth fails on relaunch (session expired) | Create new anonymous account silently, migrate all local check-in and streak data to the new `user_id`, and present only the display name selection screen. The privacy screen is NOT shown again. The user's streak and history are preserved. |
...
- Before anonymous account creation, the system must perform a device integrity check using the platform-native attestation API (Android: Play Integrity API; iOS: App Attest / DeviceCheck).
- If the integrity check fails (e.g., rooted device, emulator, tampered app), account creation is blocked.
- User-facing message on integrity failure: "This device is not supported. Please try again on a different device."
- The integrity check runs silently after the user taps "Start" and before the `signInAnonymously()` call.

### 12.3 Data Access Rules
